...
(** [decompress_lzw bic out_channel verbose] decompresses the input from [bic]
    using the LZW algorithm and writes the output to [out_channel].
    If [verbose] is true, prints detailed debug information.
...
*)
let decompress_lzw (bic : bit_in_channel) (oc : out_channel) (verbose : bool) : unit =
  ...
  let rec process_code prev_code =
    ...
    | Some curr_code ->
      ...
      if verbose then
        Printf.printf "Decompressed code: %d, bytes: %s\n" 
          curr_code
          (String.concat " " (Array.to_list (Array.map string_of_int bytes)));
...
let () =
  ...
  in
  decompress_lzw in_channel out_channel !verbose;
